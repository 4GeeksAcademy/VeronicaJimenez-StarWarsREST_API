...
def get_people():
    ...
    json = [person.serialize() for person in people]
    return json, 200
...
def get_user_favorite():
    ...
    json = [favorite.serialize() for favorite in user_favorites]
    return json, 200

@app.route('/favorite/planet/<int:planets_id>', methods=['POST'])
def post_favorite_planet(planets_id):
    new_planet = Favorites(planets_id = planets_id)
...
def post_favorite_person(people_id):
    new_person = Favorites(people_id = people_id)
...
def delete_favorite_planet(planets_id):
    planet = Favorites.query.get(planets_id)
    if planet is None:
